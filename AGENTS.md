...
| Path | Purpose | Status |
| :--- | :--- | :--- |
| `packages/seo/` | `@achimismaili/easy-web-seo` — shared SEO primitives: `easyWebSeo()` AstroIntegration (wraps `@astrojs/sitemap` with i18n hreflang, injects dynamic `robots.txt`), `<SeoHead>` component (canonical, OG, Twitter Cards, hreflang, theme-color, manifest). See [ADR 0012](../websites/docs/decisions/0012-shared-seo-primitives-in-easy-web.md). | Real at v0.2.0 |
